...
import { getCountryByCode as findCountryByCode } from "../store/useCountryStore";
...
const DetailedData = ({ country }) => {
  ...
  const cardClasses =
    "bg-card border border-card-border backdrop-blur-lg backdrop-saturate-[180%] rounded-2xl p-8 flex flex-col gap-6 relative overflow-hidden group hover:border-primary/40 transition-all duration-300";
        ...
        <div className="flex flex-col gap-4 text-sm">
          ...
          {data.borders && data.borders.length > 0 && (
            <div className="pt-3 border-t border-border-glass">
              <span className="text-text-muted text-xs font-label-mono uppercase block mb-2">
                Border Countries
              </span>

              <div className="flex flex-wrap gap-1.5">
                {data.borders.map((bCode) => {
                  const bCountry = findCountryByCode(allCountries, bCode);
      ...
      <div className={cardClasses}>
        ...
        <div className="flex flex-col gap-4 text-sm">
          ...
          <div className="mt-auto pt-4 flex flex-col gap-2 border-t border-border-glass">
            <span className="font-label-mono text-[10px] text-text-muted tracking-widest uppercase">
              RESTCOUNTRIES SCHEMA INTEGRITY
            </span>
            ...
            <span className="font-label-mono text-xs text-secondary text-right font-semibold">
